...
export class NotificacionHttpRepository implements NotificacionRepository {
  async listarPorUsuario(idUsuario: number): Promise<Notificacion[]> {
    const { data } = await apiClient.get<Notificacion[]>("/notificaciones/me/");
    return data;
  }

  async resumenPorUsuario(idUsuario: number): Promise<ResumenNotificaciones> {
  ...
  async marcarTodas(idUsuario: number): Promise<ResumenNotificaciones> {
