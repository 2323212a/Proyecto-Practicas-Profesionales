...
export function GestionVacantes() {
  ...
  async function cambiarEstado(vacante: VacanteRevision, nuevoEstado: string) {
    try {
      ...
      await gestionVacantesRevisionUseCase.cambiarEstado(vacante.id_vacante, nuevoEstado);
      await cargarVacantes();
    } catch (err: any) {
      console.error(err);
      alert(err?.response?.data?.detail ?? "No se pudo actualizar la vacante.");
    } finally {
      setGuardando(null);
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Controla que solo las vacantes de empresas activas y con cupo aparezcan disponibles para los alumnos.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        {[
          ...
          ["Activas", resumen.activas, CheckCircle2],
          ["Cerradas", resumen.cerradas, XCircle],
          ["Publicables", resumen.publicables, Clock],
            ...
            <div key={vacante.id_vacante} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
              ...
              <div className="flex flex-wrap gap-2 mt-5">
                {vacante.estado_vacante === "Activa" ? (
                  ...
                  <button
                    onClick={() => cambiarEstado(vacante, "Activa")}
                    disabled={guardando === vacante.id_vacante}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 flex items-start gap-3">
        ...
        <p className="text-sm text-[#0d2b5e]">
          Una vacante solo aparece en el padron del alumno si la empresa esta activa, la vacante esta activa y aun tiene cupo disponible.
        </p>
      </div>
    </div>
  );
