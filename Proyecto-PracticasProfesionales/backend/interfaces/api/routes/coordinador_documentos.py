...
def _listar_formatos(db: Session) -> list[FormatoDocumentoResponse]:
    formatos = (
        ...
        .filter(FormatoDocumentoModel.activo == True)  # noqa: E712
...
def obtener_dashboard_coordinador_documental(db: Session = Depends(obtener_db)):
    tipos = db.query(TipoDocumentoModel).filter(TipoDocumentoModel.obligatorio == True).all()  # noqa: E712
    alumnos = (
...
def _actualizar_estado_expediente(
    ...
    tipos_habilitantes = [
        ...
        for tipo in (
            ...
            .filter(TipoDocumentoModel.obligatorio == True)  # noqa: E712
            .all()
        )
...
def subir_formato_documento(datos: SubirFormatoRequest, db: Session = Depends(obtener_db)):
    ...
    db.query(FormatoDocumentoModel).filter(
        ...
        FormatoDocumentoModel.activo == True,  # noqa: E712
    ).update({"activo": False})
