                        ...
                        <tbody>
                          {matrizTipoUnidad.map((item, indice) => (
                            <tr key={item.id_tipo_documento_empresa} className="border-b last:border-0">
                              ...
                              <td><input type="number" value={item.orden ?? ""} onChange={(e) => setMatrizTipoUnidad((actual) => actual.map((fila, i) => i === indice ? { ...fila, orden: e.target.value ? Number(e.target.value) : null } : fila))} className="w-16 border rounded px-2 py-1" /></td>
