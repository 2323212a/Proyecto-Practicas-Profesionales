        ...
        "selecciones": [
            {
                ...
                "fecha_revision": (
                    seleccion.fecha_revision.isoformat()
                    if seleccion.fecha_revision
                    else None
                ),
            }
...
def guardar_preferencias_alumno(
    ...
    if set(ids_empresas) != empresas_activas:
        raise HTTPException(
            status_code=400,
            detail=(
                "Todas las empresas seleccionadas deben estar activas "
                "y tener convenio vigente en el padrón"
            ),
        )
